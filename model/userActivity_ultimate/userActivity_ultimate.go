...
// 插入数据
func UserActivity_ultimateInsert(insertArr map[string]interface{})  (id int64) {
	db := utils.MysqlDb
    // 1. 写SQL语句
    ...
    sqlStr := `insert into userActivity_ultimate(` + strings.Join(keysArr, ", ") + `) values(` + strings.Join(valsArr, ",") + `)`
	fmt.Println(sqlStr)
    // 2. exec
