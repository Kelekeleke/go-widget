...
// 查询单个记录
func CountryQueryOne(device_title string) (int64) {
	db := utils.MysqlDb

    var u1 country //用来接收查询结果
    // 1. 写查询单条记录的sql语句
    sqlStr := `select id from country where title=?;` //？占位 下面的id
    // 2. 执行并拿到结果
...
// 插入数据
func CountryInsert(country_title string)  (id int64) {
	db := utils.MysqlDb
    // 1. 写SQL语句
    sqlStr := `insert into country(title) values("` + country_title + `")`
    // 2. exec
